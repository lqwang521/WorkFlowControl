...
#define appWidth  self.bounds.size.width
#define appHeight  self.bounds.size.height
...
@interface HTMIWFCPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@implementation HTMIWFCPickerView
...
- (void)makeSureAndCancleButtonAndLabelTime {
    ...
    [mscButton1 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    ...
    mscButton2.frame = CGRectMake(appWidth-kW6(60), 5, kW6(60), kH6(45));
    [mscButton2 setTitle:array[1] forState:UIControlStateNormal];
    [mscButton2 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    ...
    myTimeLabel = [[UILabel alloc]initWithFrame:CGRectMake(kW6(60), 5, appWidth-kW6(120), kH6(45))];
    myTimeLabel.font = [UIFont systemFontOfSize:15];
    myTimeLabel.textAlignment = NSTextAlignmentCenter;
    [self addSubview:myTimeLabel];
    UIImageView *myImg = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, appWidth, 1)];
...
- (void)makeSureAndCancleButtonClick:(UIButton *)btn {
    ...
    if (btn.tag == 1) {
        if (pickerTypeInteger == 1) {
            self.myPickerBlockString([NSString stringWithFormat:@"%@",my_Year]);
        }else if (pickerTypeInteger == 2){
