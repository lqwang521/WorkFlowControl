//
//  HTMIWFCSegmentedControl.m
//  HTMIWFCSegmentedControl
//
...
@interface HTMIWFCHMScrollView : UIScrollView
@end

@interface HTMIWFCSegmentedControl ()
...
@property (nonatomic, strong) HTMIWFCHMScrollView *scrollView;

@end

@implementation HTMIWFCHMScrollView
...
@implementation HTMIWFCSegmentedControl
...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.type = HTMIWFCSegmentedControlTypeText;
    }
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
    ...
    if (self) {
        ...
        self.type = HTMIWFCSegmentedControlTypeImages;
    }
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
		self.sectionTitles = sectiontitles;
        self.type = HTMIWFCSegmentedControlTypeTextImages;
    }
...
- (void)commonInit {
    self.scrollView = [[HTMIWFCHMScrollView alloc] init];
    ...
    self.selectionStyle = HTMIWFCSegmentedControlSelectionStyleTextWidthStripe;
    self.selectionIndicatorLocation = HTMIWFCSegmentedControlSelectionIndicatorLocationUp;
    self.segmentWidthStyle = HTMIWFCSegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(HTMIWFCSegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
	_selectionIndicatorLocation = selectionIndicatorLocation;
	
	if (selectionIndicatorLocation == HTMIWFCSegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(HTMIWFCSegmentedControlSegmentWidthStyle)segmentWidthStyle {
    // Force HTMIWFCSegmentedControlSegmentWidthStyleFixed when type is HTMIWFCSegmentedControlTypeImages.
    if (self.type == HTMIWFCSegmentedControlTypeImages) {
        _segmentWidthStyle = HTMIWFCSegmentedControlSegmentWidthStyleFixed;
    } else {
        _segmentWidthStyle = segmentWidthStyle;
    }
}

- (void)setBorderType:(HTMIWFCSegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HTMIWFCSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            BOOL locationUp = (self.selectionIndicatorLocation == HTMIWFCSegmentedControlSelectionIndicatorLocationUp);
            BOOL selectionStyleNotBox = (self.selectionStyle != HTMIWFCSegmentedControlSelectionStyleBox);
            ...
            CGRect rect;
            if (self.segmentWidthStyle == HTMIWFCSegmentedControlSegmentWidthStyleFixed) {
            ...
            } else if (self.segmentWidthStyle == HTMIWFCSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == HTMIWFCSegmentedControlTypeImages) {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == HTMIWFCSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
    ...
    } else if (self.type == HTMIWFCSegmentedControlTypeTextImages){
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
    ...
    if (self.selectedSegmentIndex != HTMIWFCSegmentedControlNoSegment) {
        if (self.selectionStyle == HTMIWFCSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionStyle == HTMIWFCSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    // Border layer
    if (self.borderType & HTMIWFCSegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
    ...
    if (self.selectionIndicatorLocation == HTMIWFCSegmentedControlSelectionIndicatorLocationDown) {
    ...
    if (self.selectionIndicatorLocation == HTMIWFCSegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == HTMIWFCSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HTMIWFCSegmentedControlSelectionStyleTextWidthStripe &&
            sectionWidth <= self.segmentWidth &&
            self.segmentWidthStyle != HTMIWFCSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
            ...
            if (self.segmentWidthStyle == HTMIWFCSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == HTMIWFCSegmentedControlTypeText && self.segmentWidthStyle == HTMIWFCSegmentedControlSegmentWidthStyleFixed) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == HTMIWFCSegmentedControlTypeText && self.segmentWidthStyle == HTMIWFCSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == HTMIWFCSegmentedControlTypeTextImages && self.segmentWidthStyle == HTMIWFCSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == HTMIWFCSegmentedControlTypeTextImages && HTMIWFCSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        } else if (self.type == HTMIWFCSegmentedControlTypeTextImages || self.type == HTMIWFCSegmentedControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index == HTMIWFCSegmentedControlNoSegment) {
        ...
        if (animated) {
            ...
            if(self.selectionStyle == HTMIWFCSegmentedControlSelectionStyleArrow) {
                ...
                if ([self.selectionIndicatorStripLayer superlayer] == nil) {
                    ...
                    if (self.selectionStyle == HTMIWFCSegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                        [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
