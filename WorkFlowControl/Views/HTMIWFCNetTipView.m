...
@implementation HTMIWFCNetTipView
...
- (HTMIWFCNetTipView *)getNetTipView
{
    UIView *view = [self getViewWithTag:kNetTipViewTag];
    
    if (view && [view isKindOfClass:[HTMIWFCNetTipView class]]){
        
        return (HTMIWFCNetTipView *)view;
    }
    else
    {
        return nil;
...
{
    HTMIWFCNetTipView *view = [self getNetTipView];
    
    if (!view) {
        view = [[HTMIWFCNetTipView alloc] initWithFrame:self.bounds];
