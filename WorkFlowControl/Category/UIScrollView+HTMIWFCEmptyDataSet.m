//
//  UIScrollView+HTMIWFCEmptyDataSet.m
//  DZNEmptyDataSet
...
@interface HTMIWFCDZNWeakObjectContainer : NSObject
...
@interface HTMIWFCDZNEmptyDataSetView : UIView
...
#pragma mark - UIScrollView+HTMIWFCEmptyDataSet
...
@property (nonatomic, readonly) HTMIWFCDZNEmptyDataSetView *emptyDataSetView;
@end

@implementation UIScrollView (HTMIWFCDZNEmptyDataSet)

#pragma mark - Getters (Public)

- (id<DZNEmptyDataSetSource>)emptyDataSetSource
{
    HTMIWFCDZNWeakObjectContainer *container = objc_getAssociatedObject(self, kEmptyDataSetSource);
    return container.weakObject;
}

- (id<DZNEmptyDataSetDelegate>)emptyDataSetDelegate
{
    HTMIWFCDZNWeakObjectContainer *container = objc_getAssociatedObject(self, kEmptyDataSetDelegate);
    return container.weakObject;
}
...
- (HTMIWFCDZNEmptyDataSetView *)emptyDataSetView
{
    HTMIWFCDZNEmptyDataSetView *view = objc_getAssociatedObject(self, kEmptyDataSetView);
    
    if (!view)
    {
        view = [HTMIWFCDZNEmptyDataSetView new];
...
- (void)setEmptyDataSetSource:(id<DZNEmptyDataSetSource>)datasource
{
    ...
    objc_setAssociatedObject(self, kEmptyDataSetSource, [[HTMIWFCDZNWeakObjectContainer alloc] initWithWeakObject:datasource], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)setEmptyDataSetDelegate:(id<DZNEmptyDataSetDelegate>)delegate
{
    ...
    objc_setAssociatedObject(self, kEmptyDataSetDelegate, [[HTMIWFCDZNWeakObjectContainer alloc] initWithWeakObject:delegate], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}


#pragma mark - Setters (Private)

- (void)setEmptyDataSetView:(HTMIWFCDZNEmptyDataSetView *)view
{
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        HTMIWFCDZNEmptyDataSetView *view = self.emptyDataSetView;
...
void htmiWFCdzn_original_implementation(id self, SEL _cmd)
{
    // Fetch original implementation from lookup table
    Class baseClass = htmiWFCdzn_baseClassToSwizzleForTarget(self);
    NSString *key = htmiWFCdzn_implementationKey(baseClass, _cmd);
...
NSString *htmiWFCdzn_implementationKey(Class class, SEL selector)
{
    if (!class || !selector) {
        return nil;
    }
...
Class htmiWFCdzn_baseClassToSwizzleForTarget(id target)
{
...
- (void)swizzleIfPossible:(SEL)selector
{
    ...
    Class baseClass = htmiWFCdzn_baseClassToSwizzleForTarget(self);
    NSString *key = htmiWFCdzn_implementationKey(baseClass, selector);
    ...
    IMP dzn_newImplementation = method_setImplementation(method, (IMP)htmiWFCdzn_original_implementation);
...
#pragma mark - HTMIWFCDZNEmptyDataSetView

@interface HTMIWFCDZNEmptyDataSetView ()
@end

@implementation HTMIWFCDZNEmptyDataSetView
...
#pragma mark - HTMIWFCDZNWeakObjectContainer

@implementation HTMIWFCDZNWeakObjectContainer
