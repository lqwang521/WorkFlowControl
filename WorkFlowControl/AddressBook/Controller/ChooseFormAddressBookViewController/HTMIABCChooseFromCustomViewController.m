...
@interface HTMIABCChooseFromCustomViewController()<UITableViewDelegate,UITableViewDataSource>
{
    /**
     *  是否为正在搜索
     */
...
@implementation HTMIABCChooseFromCustomViewController
...
- (void)dealloc{
//    HTLog(@"%@",@"HTMIABCChooseFromCustomViewController");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.htmiAddressBookSelectedView.htmiReOrderTableView.tableView) {//已选择的TableView
        ...
        if (!_isSearch) {
            ...
            if ([self.allDataSource[indexPath.row] isKindOfClass:[HTMIABCDynamicTreeNode class]]) {
                HTMIABCDynamicTreeNode *node = self.allDataSource[indexPath.row];
                cell.htmiDynamicTreeNode = node;
            }
            else{
                ...
                //选择类型
                node.chooseType = self.chooseType;
                self.allDataSource[indexPath.row] = node;
                cell.htmiDynamicTreeNode = node;
...
- (void)initData{
    ...
    self.allDataSource = self.specificArray;
...
- (NSMutableArray *)selectedDataSource{
    if (!_selectedDataSource) {
        _selectedDataSource = [self.myParentViewController selectedDataSource];
    }
