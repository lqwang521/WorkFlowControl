//
//  HTMIWFCUzysAssetsPickerController.m
...
#import "HTMIWFCUzysAssetsViewCell.h"
#import "HTMIWFCUzysWrapperPickerController.h"
#import "HTMIWFCUzysGroupPickerView.h"
#import <ImageIO/ImageIO.h>
#import "UIImage+HTMIWFCWM.h"

@interface HTMIWFCUzysAssetsPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
//View
...
@property (nonatomic, strong) HTMIWFCUzysWrapperPickerController *picker;
@property (nonatomic, strong) HTMIWFCUzysGroupPickerView *groupPicker;
//@property (nonatomic, strong) HTMIWFCUzysGroupPickerViewController *groupPicker;
...
@implementation HTMIWFCUzysAssetsPickerController
...
- (id)init
{
    ...
    self = [super initWithNibName:@"HTMIWFCUzysAssetsPickerController" bundle:bundle];
    //self = [super initWithNibName:@"HTMIWFCUzysAssetsPickerController" bundle:[NSBundle bundleForClass:[HTMIWFCUzysAssetsPickerController class]]];
    if(self)
    {
...
{
    HTMIWFCUzysWrapperPickerController *picker = [[HTMIWFCUzysWrapperPickerController alloc] init];
...
- (void)setupLayout
{
    HTMIWFCUzysAppearanceConfig *appearanceConfig = [HTMIWFCUzysAppearanceConfig sharedConfig];
...
{
    __weak typeof(self) weakSelf = self;
    self.groupPicker = [[HTMIWFCUzysGroupPickerView alloc] initWithGroups:self.groups];
...
- (void)setupOneMediaTypeSelection
{
    if(_maximumNumberOfSelectionMedia > 0)
    {
        ...
        if(_maximumNumberOfSelection > 1)
            self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"Choose media", @"HTMIWFCUzysAssetsPickerController", nil);
        else
            self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"Choose a media", @"HTMIWFCUzysAssetsPickerController", nil);
    ...
    else
    {
        if(_maximumNumberOfSelectionPhoto == 0)
        {
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection > 1)
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"Choose videos", @"HTMIWFCUzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"Choose a video", @"HTMIWFCUzysAssetsPickerController", nil);
        }
        else if(_maximumNumberOfSelectionVideo == 0)
        {
            //            self.assetsFilter = [ALAssetsFilter allPhotos];
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection >1)
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"Choose photos", @"HTMIWFCUzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"Choose a photo", @"HTMIWFCUzysAssetsPickerController", nil);
        }
        else
        {
...
- (void)setupCollectionView
{
    ...
    [self.collectionView registerClass:[HTMIWFCUzysAssetsViewCell class]
...
- (void)setupGroup:(voidBlock)endblock withSetupAsset:(BOOL)doSetupAsset
{
    ...
    ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error) {
        ...
        [strongSelf setTitle:NSLocalizedStringFromTable(@"Not Allowed", @"HTMIWFCUzysAssetsPickerController",nil)];
        //        [self.btnTitle setTitle:NSLocalizedStringFromTable(@"Not Allowed", @"HTMIWFCUzysAssetsPickerController",nil) forState:UIControlStateNormal];
...
- (void)initNoAssetView
{
    ...
    title.text              = NSLocalizedStringFromTable(@"No Photos or Videos", @"HTMIWFCUzysAssetsPickerController", nil);
    ...
    message.text            = NSLocalizedStringFromTable(@"You can sync photos and videos onto your iPhone using iTunes.", @"HTMIWFCUzysAssetsPickerController",nil);
    ...
    //使用我们自己的bundle
    UIImageView *titleImage = [[UIImageView alloc] initWithImage:[UIImage getPNGImageHTMIWFC:@"uzysAP_ico_no_image"]];
...
- (void)showNotAllowed
{
    ...
    //使用我们自己的bundle
    UIImageView *locked     = [[UIImageView alloc] initWithImage:[UIImage getPNGImageHTMIWFC:@"uzysAP_ico_no_access"]];
    ...
    title.text              = NSLocalizedStringFromTable(@"This app does not have access to your photos or videos.", @"HTMIWFCUzysAssetsPickerController",nil);
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedStringFromTable(@"You can enable access in Privacy Settings.", @"HTMIWFCUzysAssetsPickerController",nil);
...
- (void)showNoAssetsIfNeeded
{
    ...
    voidBlock setNoImage = ^{
        ...
        //使用我们自己的bundle
        imgView.image = [UIImage getPNGImageHTMIWFC:@"uzysAP_ico_no_image"];
        
        UILabel *title = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewTitleLabel];
        title.text = NSLocalizedStringFromTable(@"No Photos", @"HTMIWFCUzysAssetsPickerController",nil);
        UILabel *msg = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewMsgLabel];
        msg.text = NSLocalizedStringFromTable(@"You can sync photos onto your iPhone using iTunes.",@"HTMIWFCUzysAssetsPickerController", nil);
    };
    voidBlock setNoVideo = ^{
        UIImageView *imgView = (UIImageView *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewImageView];
        //使用我们自己的bundle
        imgView.image = [UIImage getPNGImageHTMIWFC:@"uzysAP_ico_no_video"];
        NSLog(@"no video");
        UILabel *title = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewTitleLabel];
        title.text = NSLocalizedStringFromTable(@"No Videos", @"HTMIWFCUzysAssetsPickerController",nil);
        UILabel *msg = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewMsgLabel];
        msg.text = NSLocalizedStringFromTable(@"You can sync videos onto your iPhone using iTunes.",@"HTMIWFCUzysAssetsPickerController", nil);
    ...
    if(self.assets.count ==0)
    {
        ...
        else
        {
            if(self.maximumNumberOfSelectionMedia >0)
            {
                ...
                msg.text = NSLocalizedStringFromTable(@"You can sync media onto your iPhone using iTunes.",@"HTMIWFCUzysAssetsPickerController", nil);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HTMIWFCUzysAssetsViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CellIdentifier forIndexPath:indexPath];
...
- (void)finishPickingAssets
{
    ...
    if([assets count]>0)
    {
        HTMIWFCUzysAssetsPickerController *picker = (HTMIWFCUzysAssetsPickerController *)self;
...
- (IBAction)btnAction:(id)sender {
    ...
    switch (btn.tag) {
        ...
        {
            if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
                NSString *title = NSLocalizedStringFromTable(@"Error", @"HTMIWFCUzysAssetsPickerController", nil);
                NSString *message = NSLocalizedStringFromTable(@"Device has no camera", @"HTMIWFCUzysAssetsPickerController", nil);
