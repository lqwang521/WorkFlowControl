...
@interface HTMIWFCFMDatabasePool()

- (void)pushDatabaseBackInPool: (HTMIWFCFMDatabase*)db;
-  (HTMIWFCFMDatabase*)db;

@end


@implementation HTMIWFCFMDatabasePool
...
- (void)pushDatabaseBackInPool: (HTMIWFCFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The HTMIWFCFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
-  (HTMIWFCFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [HTMIWFCFMDatabase databaseWithPath:self->_path];
