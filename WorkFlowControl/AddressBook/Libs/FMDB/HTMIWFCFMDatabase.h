...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `<HTMIWFCFMResultSet>` - Represents the results of executing a query on an `FMDatabase`.
 - `<HTMIWFCFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<HTMIWFCFMDatabasePool>` - A pool of `FMDatabase` objects.
 - `<HTMIWFCFMStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `FMDatabase` object and use it across multiple threads. Instead, use `<HTMIWFCFMDatabaseQueue>`.
...
@interface HTMIWFCFMDatabase : NSObject  {
...
/** Execute select statement
 ...
 Executing queries returns an `<HTMIWFCFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[HTMIWFCFMResultSet next]>`) from one record to the other.
 ...
 @return A `<HTMIWFCFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see HTMIWFCFMResultSet
 @see [`HTMIWFCFMResultSet next`](<[HTMIWFCFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (HTMIWFCFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
    ...
    [queue inDatabase:^(FMDatabase *adb) {
        ...
        [adb makeFunctionNamed:@"StringStartsWithH" maximumArguments:1 withBlock:^(sqlite3_context *context, int aargc, sqlite3_value **aargv) {
            if (sqlite3_value_type(aargv[0]) == SQLITE_TEXT) {
                ...
                HTLog(@"Unknown formart for StringStartsWithH (%d) %s:%d", sqlite3_value_type(aargv[0]), __FUNCTION__, __LINE__);
                sqlite3_result_null(context);
            }
        }];

        int rowCount = 0;
        HTMIWFCFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            HTLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `HTMIWFCFMStatement`, but rather with `<FMDatabase>` and `<HTMIWFCFMResultSet>` only.
...
@interface HTMIWFCFMStatement : NSObject {
