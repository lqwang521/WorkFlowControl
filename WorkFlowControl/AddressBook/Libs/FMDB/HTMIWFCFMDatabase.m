...
@interface HTMIWFCFMDatabase ()

- (HTMIWFCFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation HTMIWFCFMDatabase
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    HTMIWFCFMDatabase *self = (__bridge HTMIWFCFMDatabase*)f;
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        HTMIWFCFMResultSet *rs = (HTMIWFCFMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(HTMIWFCFMResultSet *)resultSet {
...
- (HTMIWFCFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(HTMIWFCFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(HTMIWFCFMStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    HTMIWFCFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"HTMIWFCFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
- (HTMIWFCFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (HTMIWFCFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    HTMIWFCFMStatement *statement  = 0x00;
    HTMIWFCFMResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[HTMIWFCFMStatement alloc] init];
    ...
    rs = [HTMIWFCFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (HTMIWFCFMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (HTMIWFCFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (HTMIWFCFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (HTMIWFCFMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    HTMIWFCFMStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[HTMIWFCFMStatement alloc] init];
...
int HTMIWFCFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int HTMIWFCFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? HTMIWFCFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
void HTMIWFCFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void HTMIWFCFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &HTMIWFCFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &HTMIWFCFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
@implementation HTMIWFCFMStatement
