...
@interface HTMIABCDYSegmentControllerView()
...
@implementation HTMIABCDYSegmentControllerView
...
- (void)layout{
    switch (self.style) {
        case DYSementStyleDefault:{
            [self.containView mas_makeConstraints:^(HTMIWFCMASConstraintMaker *make) {
                make.edges.mas_equalTo(0);
                make.height.mas_equalTo(self.mas_height);
            }];
            
            [self.bottomLine mas_makeConstraints:^(HTMIWFCMASConstraintMaker *make) {
            ...
            for (UIButton *btn in self.btnArr) {
                [btn mas_makeConstraints:^(HTMIWFCMASConstraintMaker *make) {
        ...
        case DYSementStyleWidthEqualFull:{
            ...
            [lastBtn mas_makeConstraints:^(HTMIWFCMASConstraintMaker *make) {
...
- (UIColor *)selectedColor{
    if (!_selectedColor) {
        ...
        if ([[HTMIWFCSettingManager manager] navigationBarIsLightColor]) {//如果是白色色调就用蓝色
            _selectedColor = [[HTMIWFCSettingManager manager] blueColor];
        }
        else{
            _selectedColor = [[HTMIWFCSettingManager manager] navigationBarColor];
        }
    }
