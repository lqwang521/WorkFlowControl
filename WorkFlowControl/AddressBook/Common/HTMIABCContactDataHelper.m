...
#import "HTMIABCSYS_UserModel.h"//model

@implementation HTMIABCContactDataHelper
...
+ (NSMutableArray *)getFriendListDataBy:(NSMutableArray *)array{
    ...
    NSArray *serializeArray = [(NSArray *)array sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {//排序
        int i;
        NSString *strA = ((HTMIABCSYS_UserModel *)obj1).suoXie;
        NSString *strB = ((HTMIABCSYS_UserModel *)obj2).suoXie;
    ...
    for (HTMIABCSYS_UserModel *user in serializeArray) {
        char c = [user.suoXie characterAtIndex:0];
        if (!isalpha(c)) {
            [oth addObject:user];
        }
        else if (c != lastC){
...
+ (NSMutableArray *)getFriendListSectionBy:(NSMutableArray *)array{
    ...
    for (NSArray *item in array) {
        HTMIABCSYS_UserModel *user = [item objectAtIndex:0];
        char c = [user.suoXie characterAtIndex:0];
        if (!isalpha(c)) {
            c = '#';
        }
