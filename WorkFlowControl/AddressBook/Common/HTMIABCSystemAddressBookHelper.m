...
@implementation HTMIABCSystemAddressBookHelper

// 单列模式
+ (HTMIABCSystemAddressBookHelper*)shareControl
{
    static HTMIABCSystemAddressBookHelper *instance;
    @synchronized(self) {
        if(!instance) {
            instance = [[HTMIABCSystemAddressBookHelper alloc] init];
        }
    }
    return instance;
}
...
{
    return [[HTMIABCSystemAddressBookHelper shareControl] addContactName:name phoneNum:num withLabel:label];
}
...
{
    return [[HTMIABCSystemAddressBookHelper shareControl] existPhone:phoneNum];
}
