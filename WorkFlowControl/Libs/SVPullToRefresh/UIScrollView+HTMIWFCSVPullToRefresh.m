//
// UIScrollView+HTMIWFCSVPullToRefresh.m
//
...
#import "UIScrollView+HTMIWFCSVPullToRefresh.h"

//fequal() and fequalzro() from http://stackoverflow.com/a/1614761/184130
#define fequal(a,b) (fabs((a) - (b)) < FLT_EPSILON)
#define fequalzero(a) (fabs(a) < FLT_EPSILON)

static CGFloat const HTMIWFCSVPullToRefreshViewHeight = 60;

@interface HTMIWFCSVPullToRefreshArrow : UIView
...
@interface HTMIWFCSVPullToRefreshView ()

@property (nonatomic, copy) void (^pullToRefreshActionHandler)(void);

@property (nonatomic, strong) HTMIWFCSVPullToRefreshArrow *arrow;
...
@implementation UIScrollView (HTMIWFCSVPullToRefresh)
    ...
    if(!self.pullToRefreshView) {
        ...
        switch (position) {
            case SVPullToRefreshPositionTop:
                yOrigin = -HTMIWFCSVPullToRefreshViewHeight;
                break;
        ...
        HTMIWFCSVPullToRefreshView *view = [[HTMIWFCSVPullToRefreshView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, HTMIWFCSVPullToRefreshViewHeight)];
...
- (void)setPullToRefreshView:(HTMIWFCSVPullToRefreshView *)pullToRefreshView {
    [self willChangeValueForKey:@"HTMIWFCSVPullToRefreshView"];
    objc_setAssociatedObject(self, &UIScrollViewPullToRefreshView,
                             pullToRefreshView,
                             OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"HTMIWFCSVPullToRefreshView"];
}

- (HTMIWFCSVPullToRefreshView *)pullToRefreshView {
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, HTMIWFCSVPullToRefreshViewHeight);
        }
    }
}
...
@implementation HTMIWFCSVPullToRefreshView
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    if (self.superview && newSuperview == nil) {
        ...
        if (scrollView.showsPullToRefresh) {
            if (self.isObserving) {
                //If enter this branch, it is the moment just before "HTMIWFCSVPullToRefreshView's dealloc", so remove observer here
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, HTMIWFCSVPullToRefreshViewHeight);
    }
...
- (HTMIWFCSVPullToRefreshArrow *)arrow {
    if(!_arrow) {
		_arrow = [[HTMIWFCSVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
#pragma mark - HTMIWFCSVPullToRefreshArrow

@implementation HTMIWFCSVPullToRefreshArrow
